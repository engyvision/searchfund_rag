...
This module implements the LLM provider interface for Perplexity AI.
"""
...
# Initialize logger
logger = get_logger("llm.providers.perplexity")
...
class PerplexityProvider(BaseLLMProvider):
    ...
    DEFAULT_API_KEY_ENV: ClassVar[str] = "PERPLEXITY_API_KEY"
    
    # Static provider name
    PROVIDER_NAME: ClassVar[str] = "perplexity"
    
    # Default model names
    DEFAULT_COMPLETION_MODEL = "pplx-7b-online"
    DEFAULT_CLARIFICATION_MODEL = "pplx-7b-online"
    
    def __init__(self, config: Dict[str, Any]):
        """Initialize the Perplexity AI provider from configuration.
        ...
        # Get model names from config (note: Perplexity doesn't support embeddings)
        self.embedding_model = ""  # Not used
        ...
        logger.info(f"Initialized PerplexityProvider with models: completion={self.completion_model}, clarification={self.clarification_model}")
    ...
    def _get_api_key_from_config(self, config: Dict[str, Any]) -> str:
        ...
        for function_type, function_config in config.items():
            if function_config.get("provider") == "perplexity" and "api_key" in function_config:
                return function_config["api_key"]
        
        # If not found in config, try environment variable
        api_key = os.getenv(self.DEFAULT_API_KEY_ENV)
        if not api_key:
            raise ValueError(f"Perplexity API key not found in config or environment variable {self.DEFAULT_API_KEY_ENV}")
    ...
    def _get_model_for_function(self, config: Dict[str, Any], function_type: str, default_model: str) -> str:
        ...
        Args:
            config: Provider configuration dictionary
            function_type: Function type (answer_generation, query_clarification, etc.)
        ...
        if function_type in config and config[function_type].get("provider") == "perplexity":
    ...
    def get_embedding(self, text: str) -> Tuple[List[float], Optional[int]]:
        ...
        Note: Perplexity doesn't offer embedding API, so we raise an error.
        ...
        Raises:
            NotImplementedError: Always raised as Perplexity doesn't support embeddings
        """
        logger.error("Perplexity does not support embeddings API")
        raise NotImplementedError("Perplexity does not support embeddings API")
    ...
    def _make_perplexity_request(self, messages: List[Dict[str, str]], model: str, max_tokens: int, temperature: float = 0.7) -> Dict:
    ...
    def generate_answer(
        ...
        try:
            ...
            response = self._make_perplexity_request(
                messages=messages,
    ...
    def clarify_query(
        ...
        **Example 3:**
        ...
        try:
            ...
            clarified_query = response.get("choices", [{}])[0].get("message", {}).get("content", "").strip()
            if not clarified_query:
                logger.error("Empty response from Perplexity API for query clarification")
                return query
    ...
    def rerank(
        ...
        try:
            ...
            if not result_text:
                logger.error("Empty response from Perplexity API for reranking")
            ...
            try:
                ...
                logger.error(f"Error parsing JSON from reranking response: {e}")
